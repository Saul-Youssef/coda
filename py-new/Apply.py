...
def aq_1(context,domain,A,B):
    if context.atom(A): return data(A|B)
...
def apby(context,domain,A,B):
    AL,AR = A.split()
    import Number
    if context.atom(AL) and len(Number.ints(AL))==1:
        n = Number.ints(AL)[0]
        if n>0:
#            if all([b.atom() for b in B[:n]]):
            if all([context.catom(b) for b in B[:n]]):
...
def ap2_1(context,domain,A,B):
    A0,AR =  A.split()
    A1,AR = AR.split()
    if context.atom(A0) and context.atom(A1): return ((A0+A1)|B) + ((domain+A0+AR)|B)
