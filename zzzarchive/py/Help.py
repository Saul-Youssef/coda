...
def help(A,B):
    I,R = B.split()
    if I.atom():
        if data(colon(I,data())) in DEF:
            H = Help(str(I))
            H.display()
            return data()
        else:
            return one(b'defs',data(),I)
def help_0(A,B):
    if B.empty(): return one(b'help',data(),data(b'help'))
DEF.add(data(b'help'),help,help_0)
#
#
def sources(A,B):
...
#
def defs(A,B):
    modules = [b.decode() for b in B if is_code(b)]
    table = []
    for flag,definitions in DEF.defs():
        fl = str(flag)
        H = Help(fl)
        if not '(' in fl and not fl.endswith('1') and not fl.endswith('_') and len(H.summary())>2:
            if len(modules)==0 or H.module() in modules:
                table.append([H.module(),H.flag(),H.summary(),len(definitions)])
    deftable(table)
    return data()
DEF.add(data(b'defs'),defs)
...
if __name__=='__main__':
    f = open('/Users/youssef/math/co/Number.co','r')
    txt = f.read()
    S = SourceFile('/Users/youssef/math/co/Number.co')
    for b in S: b.display()
    T = SourceFile('/Users/youssef/math/py/Basic.py')
